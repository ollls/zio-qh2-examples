...
object NioApp extends ZIOAppDefault {
  ...
  val filter: WebFilter[Any] = (r: Request) =>
    ZIO
      .succeed(
         Either.cond( r.uri.getPath().endsWith("test70.jpeg"), r, Response.Error(StatusCode.Forbidden).asText("Denied: " + r.uri.getPath()) ) )
  ...
  val R: HttpRouteIO[String] = {
    ...
    case req @ GET -> Root / "hello" / "1" / "2" / "user2" :? param1(
      ...
      ZIO.succeed(
        Response.Ok().asText("param1=" + test + "  " + "param2=" + test2)
      )
    ...
    case req @ POST -> Root / "mpart" =>
      MultiPart.writeAll(req, "/Users/user000/tmp1/") *> ZIO.succeed(
        Response.Ok()
      )
    ...
    case GET -> Root / "example" =>
      ...
      val ts = ZStream.fromChunks(
        Chunk.fromArray("Block1\n".getBytes()),
        Chunk.fromArray("Block22\n".getBytes())
      )
    ...
    case GET -> Root / StringVar(file) =>
      ...
      for {
        ...
        _ <- ZIO
          .fail(new java.io.FileNotFoundException(jpath.toString()))
          .when(present == false)
      } yield (Response
        .Ok()
